...
const StatisticDetailText = ({ text, isRightColumn = false }) => {
  return (
    <p
      className={"statistic-detail-text"}
      style={isRightColumn ? { float: "right", fontWeight: 600 } : null}
    >
      {text}
    </p>
  );
...
const BetHistoryStatistic = ({ data }) => {
  ...
  const totalBetAmount = data
    ...
    .reduce((prev, next) => prev + next);
  const totalPotentialProfit = data
    .map((ele) => ele.potentialProfit)
    .reduce((prev, next) => prev + next);
  ...
  return (
    <div className={"bet-history-statistic-wrapper"}>
      ...
      <StatisticDetailRow
        left={`Số Trận Chưa Hoàn Tất:`}
        right={totalUnfinished}
      />
      <StatisticDetailRow
        left={`Tổng Tiền Cược:`}
        right={`${totalBetAmount.toLocaleString()}đ`}
      />
      <StatisticDetailRow
        left={`Lợi Nhuận Tiềm Năng:`}
        right={`${totalPotentialProfit.toLocaleString()}đ`}
      />
      {hasAtLeastOneResult && (
        <>
          <Divider />
