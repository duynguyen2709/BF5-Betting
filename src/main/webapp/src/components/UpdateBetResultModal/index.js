...
const UpdateBetResultModal = ({ data, isOpen, onUpdateSuccess, onClose }) => {
  ...
  const handleChangeMatchResult = useCallback(
    (eventId, value) => {
      ...
      {isAccumulatorBet(data) &&
        data.events.map((event, index) => {
          return (
            <Row
              key={event.id}
              style={{ alignItems: "center", margin: "1rem 0.5rem 0 0.5rem" }}
            >
              ...
              <Col span={19} offset={1}>
                ...
                >
                  {Object.values(BET_RESULT)
                    .filter(
                      (res) => res.result !== BET_RESULT.Unfinished.result
                    )
                    .map((ele) => (
                      <Option key={ele.result} value={ele.result}>
                        {ele.text}
                      </Option>
                    ))}
                </Select>
