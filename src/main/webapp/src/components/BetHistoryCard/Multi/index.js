      ...
      {data.map((bet, index) => {
        ...
        return (
          <React.Fragment key={bet.betId}>
            <BetHistoryDetailRow
              left={"Loại Cược:"}
              right={parseBetEvent(event)}
            />
            <BetHistoryDetailRow
              left={"Tiền Cược:"}
              right={`${bet.betAmount.toLocaleString()}đ`}
            />
            ...
            <BetHistoryDetailRow
              left={"Kết Quả:"}
              right={<BetResultTag result={bet.result} />}
            />
            {bet.result === BET_RESULT.Win.result ||
            bet.result === BET_RESULT.HalfWin.result ? (
              <BetHistoryDetailRow
                left={"Tiền Lời:"}
                right={`${bet.actualProfit.toLocaleString()}đ`}
              />
            ) : null}
            {bet.result === BET_RESULT.Lost.result ||
            bet.result === BET_RESULT.HalfLost.result ? (
              <BetHistoryDetailRow
                left={"Tiền Lỗ:"}
                right={`${bet.actualProfit.toLocaleString()}đ`}
              />
