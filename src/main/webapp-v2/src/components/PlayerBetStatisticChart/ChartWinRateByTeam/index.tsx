...
export const ChartWinRateByTeam: React.FC<{
  ...
  const max = Math.max(winRate.totalWin, winRate.totalLost, winRate.totalDraw, winRate.totalUnfinished) + 1
  ...
  return (
    <div className={styles['row-chart-win-rate-by-team']}>
      ...
      {selectedTeam && <Chart options={chartData.options} series={chartData.series} type='bar' height={296} />}
    </div>
  )
}
